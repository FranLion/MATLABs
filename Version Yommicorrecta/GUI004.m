...
FiltroInverso = uicontrol(FigRI,'Style','radiobutton','String','Filtro inverso','Position',[300,43,100,25]);
   
    % Pulsables
ReproducirRR = uicontrol(FigRI,'Style','pushbutton','String','Reproducir','Position',...
    [300,335,150,30],'Callback',{@ruidorosa,DuracionRR});

ReproducirRI = uicontrol(FigRI,'Style','pushbutton','String','Reproducir y grabar','Position',...
    [300,70,150,50],'Callback',{@adquisicionRI,DuracionRI,Frecuenciainicial,Frecuenciafinal,Nombredearchivo,FiltroInverso});
...
GraficosSS.Callback = {@GraficosexchangeSS,DuracionRI,Frecuenciainicial,Frecuenciafinal};

GraficosRR = uicontrol(FigRI,'Style', 'popupmenu',...
    'String',{'Tiempo vs Amplitud','Frecuencia vs Presion'},'Position',[300,300,150,25]);

GraficosRR.Callback = {@GraficosexchangeRR,DuracionRR};

FiltroInverso.Callback = {@adquisicionRI,FiltroInverso};
...
FiltroInverso.Units = 'normalized';
...
% Funciones
    ...
    function GraficosexchangeSS(~,~,DuracionRI,Frecuenciainicial,Frecuenciafinal,GraficosSS,Callbackdata,event,hObject)
        
        if  GraficosSS.Value == 1
        ...
        function GraficosexchangeRR(~,~,DuracionRR,GraficosRR)
            
            if  GraficosRR.Value == 1
...
% %       ESTO ES PARA ACTUALIZAR LOS DISP DE AUDIO TODAVIA NO ANDA
% %
% %     function devicerefreshin(~,~)
% %         devices = audioDeviceWriter;
% %         devices = transpose(devices);
% %         set(devices,'String');
% %         clear all
% %
% %     end
% %
% %     function devicerefreshout(~,~)
% %
% %         devices = audioDeviceWriter;
% %         devices = transpose(devices);
% %         set(devices,'String');
